...
export default function Post(
    ...
    if (showModal)
        return <div className="w-max h-max" onClick={() => setShowModal(false)}>
            <ModalWrapper>
                <div className="w-[90lvw] h-[80lvh] rounded-2xl grid grid-cols-6 grid-rows-10 bg-black text-white relative gap-1 p-2 overflow-scroll" onClick={onClickContainer} >
                    {/* TOP */}
                    ...
                    {/* IMAGE - BOTTOM RIGHT  */}
                    <div className="col-span-full row-[2/6] md:col-span-4 md:row-start-2 md:row-span-8 ">
                        <div className="w-full h-full relative">
                            <Image className="relative" src={image} objectFit="contain" fill alt="none" />
                        </div>
                    </div>
                    {/* TEXT - BOTTOM LEFT */}
                    <div className="col-span-full md:col-[5/7] row-span-8 grid grid-rows-10 grid-cols-3 ">
                        <div className=" col-span-full  row-span-2 md:row-span-1 grid grid-cols-3 grid-rows-1 content-center">
                            ...
                            <div className="col-start-4 flex px-2">
                                <LikeButton id={id} like={like} length={likes} />
                                <View view={view} />
                            </div>
                        </div>
                        <div className=" col-span-full row-span-2 content-center">
                        ...
                        <div className=" col-span-full row-span-4 md:row-span-3 md:p-4">
                            <p className="w-max font-bold rounded my-1 px-2 text-white">{title}</p>
                            <p className="w-full h-full px-4 my-1 text-white">{description}</p>
                        </div>
                        <div className="col-span-full row-span-2 md:row-span-3">
